...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const katet1 = Math.abs(x1) + Math.abs(x2);
  const katet2 = Math.abs(y1) + Math.abs(y2);
  const hipotinuze = katet1 ** 2 + katet2 ** 2;
  const result = Math.sqrt(hipotinuze);
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const at1 = Math.atan2(y2, x2);
  const at2 = Math.atan2(y1, x1);
  return Math.abs(at1 - at2);
}
...
function getParallelepipedDiagonal(a, b, c) {
  const adcSqrt = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(adcSqrt);
}
...
function toNumber(value, def) {
  const numberOrNo = parseFloat(value);
  if (Number.isNaN(numberOrNo)) {
    return def;
  }
  return numberOrNo;
}
